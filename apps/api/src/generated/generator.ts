...
async function readSyllabusData() {
    ...
    try {
        if (!fs.existsSync(universitiesDir)) {
            throw new Error('University data directory not found. Please ensure the `universities` folder exists at the root of your project.');
        }
        ...
        await Bun.write(path.join(process.cwd(), "src/generated/university.json"), JSON.stringify(data))
    } catch (error) {
        // :)
        throw error
    }
