...
export const ChatWithSyllabusInputSchema = z.object({
  syllabusContent: z
    .string()
    .describe(
      "The syllabus/module content in markdown format that provides context for the conversation."
    ),
  moduleTitle: z
    .string()
    .optional()
    .describe("The title of the current module/syllabus being discussed."),
});
export type ChatWithSyllabusInput = z.infer<typeof ChatWithSyllabusInputSchema>;

export const ChatWithSyllabusOutputSchema = z.object({
...
const chatTutorPrompt = Chat.definePrompt({
...
IMPORTANT: Base all your responses strictly on the provided syllabus content. Do not use any cached or stored information from previous sessions.`,
  prompt: `You are helping a student with their studies. Here is the current syllabus/module content for reference:
...
Please provide a helpful response based ONLY on the syllabus content provided above. Include 3-4 follow-up questions to continue the learning conversation.`,
});

const chatWithSyllabusFlow = Chat.defineFlow(
  {
